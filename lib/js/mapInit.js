...
function loadScript() {
	...
	gmapScript.onload = function() {
		...
		google.maps.event.addListener(gmap, 'click', function (gmapClickEvent) {
            coordtrans(gmapClickEvent.latLng.lng(), gmapClickEvent.latLng.lat(), 'gcj02');
...
bmap.addEventListener("click",function(bmapClickEvent){
	coordtrans(bmapClickEvent.point.lng, bmapClickEvent.point.lat, 'bd09');
	var wgs84result = '<li>经纬坐标:<br />' + wgs84lng + ',' + wgs84lat + '</li>';
	var gcj02result = '<li>国测坐标:<br />' + gcj02lng + ',' + gcj02lat + '</li>';
	var bd09result = '<li>百度坐标:<br />' + bd09lng + ',' + bd09lat + '</li>';
...
var amapClickEventListener = amap.on('click', function(amapClickEvent) {
	coordtrans(amapClickEvent.lnglat.getLng(), amapClickEvent.lnglat.getLat(), 'gcj02');
	var wgs84result = '<li>经纬坐标:<br />' + wgs84lng + ',' + wgs84lat + '</li>';
	var gcj02result = '<li>国测坐标:<br />' + gcj02lng + ',' + gcj02lat + '</li>';
	var bd09result = '<li>百度坐标:<br />' + bd09lng + ',' + bd09lat + '</li>';
...
qq.maps.event.addListener(tmap, 'click', function(tmapClickEvent) {
	coordtrans(tmapClickEvent.latLng.lng, tmapClickEvent.latLng.lat, 'gcj02');
	var wgs84result = '<li>经纬坐标:<br />' + wgs84lng + ',' + wgs84lat + '</li>';
	var gcj02result = '<li>国测坐标:<br />' + gcj02lng + ',' + gcj02lat + '</li>';
	var bd09result = '<li>百度坐标:<br />' + bd09lng + ',' + bd09lat + '</li>';
