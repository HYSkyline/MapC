...
function lnglatQueryOutput(panelName) {
	...
	var resultWGSdiv = createResultDiv("li", wgs84lng, wgs84lat);
	var resultGCJdiv = createResultDiv("li", gcj02lng, gcj02lat);
	var resultBDdiv = createResultDiv("li", bd09lng, bd09lat);
...
function addressQueryOutput(i, name, address, lng, lat, coordtype, resultsDiv) {
	...
	var resultWGSdiv = createResultDiv("p", wgs84lng, wgs84lat);
	var resultGCJdiv = createResultDiv("p", gcj02lng, gcj02lat);
	var resultBDdiv = createResultDiv("p", bd09lng, bd09lat);

	resultsDiv.appendChild(resultWGSdiv);
	resultsDiv.appendChild(resultGCJdiv);
	resultsDiv.appendChild(resultBDdiv);
}
function createResultDiv(panelClassName, lng, lat) {
